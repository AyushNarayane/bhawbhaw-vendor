  ...
  return (
    ...
    <nav className="bg-white shadow-md mb-2 sm:px-20 px-5 p-4 w-full flex justify-between items-center">
      <div className="flex items-center">
        <img src="/logoHeader.png" alt="Logo" className="w-20 h-10 mr-2" />
      </div>
          ...
          {[1, 2, 3, 4, 5].map((stepNumber) => (
            <React.Fragment key={stepNumber}>
              <div className={`text-center lg:w-auto xs:w-10 w-8 lg:mx-0 sm:mx-7 xs:mx-3 mx-2 ${getStepStyle(stepNumber).textColor}`}>
                <div
                  className={`sm:w-12 sm:h-12 w-8 h-8 mx-auto rounded-full flex items-center justify-center border-2 ${getStepStyle(stepNumber).borderColor} ${
                    visitedSteps.includes(stepNumber) ? getStepStyle(stepNumber).iconBg : ''
                  } ${getStepStyle(stepNumber).bgColor}`}
                >
              ...
              {stepNumber < 5 && (
                <div
                  className={`w-full mb-10 ${step > stepNumber ? 'bg-black h-1' : 'bg-gray-400 h-0.5'}`}
                />
              )}
            </React.Fragment>
          ))}
